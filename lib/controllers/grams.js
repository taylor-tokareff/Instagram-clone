...
export default Router()
  ...
  .delete('/api/v1/grams/:id', ensureAuth, async (req, res) => {
    try {
  ...
  .put('/api/v1/grams/:id', ensureAuth, async (req, res) => {

    try {
      const gram = await Gram.update(req.params.id, { ...req.body, userId: req.user.id });
      res.send(gram);
    } catch (err) {
      res.status(500).send({ error: err.message });
    }
